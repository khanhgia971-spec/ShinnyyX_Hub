...
local currentIslandName="Prehistoric Island"
local ISLAND_POSITIONS={
    ["Prehistoric Island"]=Vector3.new(300,10,400),
    ["Frozen Dimension"]=Vector3.new(200,10,200),
    ["Sea of Treats"]=Vector3.new(800,10,900),
    ["Tiki Outpost"]=Vector3.new(400,15,500),
    ["Hydra Island"]=Vector3.new(200,20,300)
}
...
local function startVolcanoEvent()
    ...
    if skullPos then
        ...
        volcanoPressure=0
        relicHealth=100
        print("[VolcanoEvent] Sự kiện núi lửa đã bắt đầu!")
        return true
    end
    return false
end
...
local function collectFireFlower(flower)
    if not flower then return false end
    if not rootPart then return false end
    local dist=getDistance(rootPart.Position,flower.Position)
    if dist>5 then
        moveToPosition(flower.Position,3)
    end
    if dist<5 then
        flower:Destroy()
        return true
...
local function collectAllFireFlowers()
    local flowers=findFireFlowers()
    local count=0
    for _,flower in ipairs(flowers) do
        if collectFireFlower(flower) then
            count=count+1
        end
        wait(0.1)
    end
...
local function checkEventRequirements()
    local level=getPlayerLevel()
    local sea=getCurrentSea()
    if sea<2 then
        print("[VolcanoEvent] Cần Sea 2 hoặc 3 để vào đảo Prehistoric")
        return false
    end
    return true
end
local function processVolcanoEvent(data)
    ...
    if not checkEventRequirements() then return end
    if not volcanoIsland or not volcanoIsland.Parent then
        ...
        if not volcanoIsland then
            if data.autoFind then
                ...
                -- Di chuyển đến vị trí đảo Prehistoric nếu biết
                local pos=ISLAND_POSITIONS["Prehistoric Island"]
                if pos and getDistance(rootPart.Position,pos)>200 then
    ...
    if eventActive and not eventCompleted then
        ...
        if timeRemaining<=0 or relicHealth<=0 then
            eventCompleted=true
            eventActive=false
            print("[VolcanoEvent] Sự kiện hoàn thành!")
...
function volcanoEvent.CollectFireFlower(flower)
    return collectFireFlower(flower)
end
function volcanoEvent.CollectAllFireFlowers()
    return collectAllFireFlowers()
end
...
function volcanoEvent.GetFireFlowers()
...
function volcanoEvent.GetIslandPosition(name)
    return ISLAND_POSITIONS[name]
end
function volcanoEvent.SetIslandPosition(name,pos)
    ISLAND_POSITIONS[name]=pos
    return true
end
...
function volcanoEvent.Initialize(data)
    ...
    if data then
        ...
        if data.autoMove==nil then data.autoMove=true end
        if data.islandPosition then
            ISLAND_POSITIONS["Prehistoric Island"]=data.islandPosition
        end
    end
    return true
end
