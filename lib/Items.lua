...
local FRUIT_LIST={
    -- Common
    "Bomb","Spike","Rubber","Chop","Falcon","Smoke","Spring",
    -- Uncommon
    "Flame","Ice","Sand","Ghost","Diamond","Spider","Love",
    -- Rare
    "Dark","Light","Magma","Buddha","Revive","Door","Paw","Phoenix",
    -- Legendary
    "Quake","String","Gravity","Shadow","Rumble","Blizzard","Sound","Pain",
    -- Mythical
    "Dragon","Leopard","Venom","Control","Soul","Mammoth","Kitsune","T-Rex",
    "Dough","Spirit","Gas","Gravity","Shadow","Rumble","Phoenix","Blizzard"
}
local WEAPON_LIST={
    "Saber","Katana","Trident","Pole","Gun","Flintlock","Musket","Cannon",
    "Sword","Axe","Hammer","Spear","Dagger","Bow","Crossbow",
    "Dual Flintlock","Skull Guitar","Acidum Rifle","Bizarre Revolver",
    "Venom Bow","Dragonstorm","Kabucha","Leviathan Crossbow",
    "Spikey Trident","Triple Katana","Hallow Scythe","Dark Dagger",
    "Soul Cane","Rengoku","Shisui","Yama","Tushita","Koko"
}
local ITEM_LIST={
    "Money","Gem","Fragment","Token","Key","Chest","Potion","Scroll",
    "Map","Compass","Dragon Egg","Dinosaur Bone","Dragon Scale","Blaze Ember",
    "Leviathan Heart","Volcanic Orb","Fire Flower","Mysterious Fruit"
}
local RARITY_COLORS={
...
local function getFruitRarity(fruitName)
    local rarities={
        ...
        ["Diamond"]="Uncommon",["Spider"]="Uncommon",["Love"]="Uncommon",
        ["Dark"]="Rare",["Light"]="Rare",["Magma"]="Rare",["Buddha"]="Rare",
        ["Revive"]="Rare",["Door"]="Rare",["Paw"]="Rare",["Phoenix"]="Rare",
        ["Quake"]="Legendary",["String"]="Legendary",["Gravity"]="Legendary",
        ["Shadow"]="Legendary",["Rumble"]="Legendary",["Blizzard"]="Legendary",
        ["Sound"]="Legendary",["Pain"]="Legendary",
        ["Dragon"]="Mythical",["Leopard"]="Mythical",["Venom"]="Mythical",
        ["Control"]="Mythical",["Soul"]="Mythical",["Mammoth"]="Mythical",
        ["Kitsune"]="Mythical",["T-Rex"]="Mythical",["Dough"]="Mythical",
        ["Spirit"]="Mythical",["Gas"]="Mythical"
    }
...
local function getWeaponRarity(weaponName)
    local rarities={
        ...
        ["Dual Flintlock"]="Rare",["Skull Guitar"]="Legendary",
        ["Acidum Rifle"]="Rare",["Bizarre Revolver"]="Rare",
        ["Venom Bow"]="Legendary",["Dragonstorm"]="Mythical",
        ["Kabucha"]="Rare",["Leviathan Crossbow"]="Legendary",
        ["Spikey Trident"]="Rare",["Triple Katana"]="Legendary",
        ["Hallow Scythe"]="Legendary",["Dark Dagger"]="Rare",
        ["Soul Cane"]="Rare",["Rengoku"]="Legendary",
        ["Shisui"]="Legendary",["Yama"]="Legendary",
        ["Tushita"]="Legendary",["Koko"]="Legendary"
    }
    return rarities[weaponName] or "Common"
end
local function getItemRarity(itemName)
    local rarities={
        ["Money"]="Common",["Gem"]="Uncommon",["Fragment"]="Rare",
        ["Token"]="Rare",["Key"]="Uncommon",["Chest"]="Common",
        ["Potion"]="Uncommon",["Scroll"]="Rare",["Map"]="Common",
        ["Compass"]="Common",["Dragon Egg"]="Legendary",
        ["Dinosaur Bone"]="Rare",["Dragon Scale"]="Legendary",
        ["Blaze Ember"]="Rare",["Leviathan Heart"]="Mythical",
        ["Volcanic Orb"]="Mythical",["Fire Flower"]="Uncommon",
        ["Mysterious Fruit"]="Legendary"
    }
    return rarities[itemName] or "Common"
end
local function spawnPart(name,position,color,size,value)
    ...
    tag.Value=value or ""
    tag.Parent=part
    local rarity=getItemRarity(name)
    local color3=RARITY_COLORS[rarity] or Color3.fromRGB(255,255,255)
...
local function spawnFruit(fruitName,position)
    ...
    local color=RARITY_COLORS[rarity] or Color3.fromRGB(255,255,0)
...
local function spawnWeapon(weaponName,position)
    ...
    local color=RARITY_COLORS[rarity] or Color3.fromRGB(0,100,255)
    local rarityLabel=Instance.new("StringValue")
    rarityLabel.Name="Rarity"
    rarityLabel.Value=rarity
    rarityLabel.Parent=part
...
local function getPlayerFragments()
    local frags=player:FindFirstChild("Fragments")
    if frags and frags:IsA("NumberValue") then
        return frags.Value
    end
    return 0
end
...
local function spawnRandomFruit()
    local fruit=FRUIT_LIST[math.random(1,#FRUIT_LIST)]
    return spawnFruit(fruit)
end
local function spawnRandomWeapon()
    local weapon=WEAPON_LIST[math.random(1,#WEAPON_LIST)]
...
local function spawnDragonEgg(position)
    position=position or rootPart.Position+Vector3.new(0,5,0)
    return spawnPart("Dragon Egg",position,Color3.fromRGB(255,0,0),3,"Dragon Egg")
end
local function spawnDinosaurBone(position)
    position=position or rootPart.Position+Vector3.new(0,5,0)
    return spawnPart("Dinosaur Bone",position,Color3.fromRGB(200,200,200),2,"Dinosaur Bone")
end
local function spawnDragonScale(position)
    position=position or rootPart.Position+Vector3.new(0,5,0)
    return spawnPart("Dragon Scale",position,Color3.fromRGB(0,200,255),2,"Dragon Scale")
end
local function spawnBlazeEmber(position)
    position=position or rootPart.Position+Vector3.new(0,5,0)
    return spawnPart("Blaze Ember",position,Color3.fromRGB(255,100,0),1.5,"Blaze Ember")
end
local function spawnLeviathanHeart(position)
    position=position or rootPart.Position+Vector3.new(0,5,0)
    return spawnPart("Leviathan Heart",position,Color3.fromRGB(255,0,255),3,"Leviathan Heart")
end
local function spawnVolcanicOrb(position)
    position=position or rootPart.Position+Vector3.new(0,5,0)
    return spawnPart("Volcanic Orb",position,Color3.fromRGB(255,50,0),2,"Volcanic Orb")
end
function items.Stop()
    isRunning=false
    return true
...
function items.SpawnDragonEgg(position)
    return spawnDragonEgg(position)
end
function items.SpawnDinosaurBone(position)
    return spawnDinosaurBone(position)
end
function items.SpawnDragonScale(position)
    return spawnDragonScale(position)
end
function items.SpawnBlazeEmber(position)
    return spawnBlazeEmber(position)
end
function items.SpawnLeviathanHeart(position)
    return spawnLeviathanHeart(position)
end
function items.SpawnVolcanicOrb(position)
    return spawnVolcanicOrb(position)
end
...
function items.GetFragments()
    return getPlayerFragments()
end
...
function items.GetItemRarity(itemName)
    return getItemRarity(itemName)
end
